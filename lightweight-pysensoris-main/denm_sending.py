...
def arg_parse():
    ...
    parser.add_argument('-sb', "--save-bin", required=False, default=0, type=int,
                    help='ros node name, cannot be the same as other existing ros nodes')

    parser.add_argument('-mt', "--mqtt-topic", required=False, default="sensoris_5GoIng/mqtt/traffic_events", type=str,
...
def main():
    ...
    ros_listener = importlib.import_module("denm_ros_listener")
    sensoris_client = importlib.import_module("denm_sensoris_client")

    q = Queue()

    client = getattr(sensoris_client, "run_sed")
    client(q, target_ip=args.target_ip, port=args.port_number, mqtt_topic=args.mqtt_topic)

    listener = getattr(ros_listener, "run")
    listener(args.node_name, q, args.topic_name, args.save_bin)
